...
import { createMcpNotifyServer } from "./mcp-notify-server.js";

export interface McpNotifyTunnel {
  remotePort: number;
  close(): void;
}

export async function setupMcpNotifyTunnel(
  sshSession: SshSession,
  broadcastChatMessage: (memberIds: string[], conversationId: string, message: unknown) => void,
  opts?: { remotePort?: number },
): Promise<McpNotifyTunnel> {
  // 1. Start local MCP HTTP server for notifications
  const mcpServer = await createMcpNotifyServer(broadcastChatMessage);
  ...
  console.log(`[mcp-notify-tunnel] Reverse tunnel: VPS 127.0.0.1:${remotePort} → local 127.0.0.1:${localPort}`);
  ...
  sshSession.onTcpConnection((info, accept) => {
    ...
    localConn.on("error", (err) => {
      console.error(`[mcp-notify-tunnel] Local connect error: ${err.message}`);
      try { sshChannel.close(); } catch {}
    });

    sshChannel.on("error", (err: unknown) => {
      const message = err instanceof Error ? err.message : String(err);
      console.error(`[mcp-notify-tunnel] SSH channel error: ${message}`);
      localConn.destroy();
    });
  });
  ...
  return {
    ...
    close() {
      ...
      mcpServer.close();
      console.log(`[mcp-notify-tunnel] Tunnel closed (remote port ${remotePort})`);
    },
  };
}
