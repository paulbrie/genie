 ...
 *  see — the assistant must never become a confused deputy with manager-wide
 *  access. */
...
/** Returns an error string to short-circuit a project tool when the caller may
 *  not access `projectId`, or null when access is allowed. Privileged roles
 *  (tazcloud/admin/superadmin) bypass, matching the WS handlers. */
async function denyIfNoProjectAccess(
  ctx: ToolAuthContext,
  projectId: string,
): Promise<string | null> {
  if (isPrivilegedRole(ctx.role)) return null;
  if (await projectService.userCanSeeProject(ctx.userId, projectId)) return null;
  return `Error: you don't have access to project "${projectId}".`;
}

/** Build the assistant toolset scoped to one caller. Project/VM tools enforce
 *  per-user access; cloud (Taz/DO) and recipe-authoring tools — which touch
 *  account-wide infra that isn't project-scoped — are only handed to privileged
 *  roles, so a plain user's assistant simply can't call them. */
export function createTools(ctx: ToolAuthContext) {
  const base = {
  web_search: tool({
    description:
  ...
  read_project_file: tool({
    ...
    execute: async ({ projectId, fileName }) => {
      const denied = await denyIfNoProjectAccess(ctx, projectId);
  ...
  ssh_exec: tool({
    ...
    execute: async ({ projectId, instance, command, timeoutSeconds }) => {
      ...
      return executeSshExec(projectId, instance, command, timeout, ctx);
    },
  }),
  ...
  read_project_doc: tool({
    ...
    execute: async ({ docId }) => {
      ...
      // A doc is reachable if you can see its project, or (for a project-less
      // personal doc) you own it. Privileged roles bypass.
      if (!isPrivilegedRole(ctx.role)) {
        const ok = doc.projectId
          ? await projectService.userCanSeeProject(ctx.userId, doc.projectId)
          : doc.ownerId === ctx.userId;
        if (!ok) return `Error: you don't have access to doc (id: ${docId}).`;
      }
      return `# ${doc.title}\n\n${doc.content}`;
    },
  }),
  ...
  // Cloud (TazCloud / DigitalOcean) and recipe-authoring tools touch account-wide
  // infrastructure that isn't scoped to a single project the caller owns — e.g.
  // tazcloud_list_vms returns every VM on the account. There's no per-user filter
  // that makes those safe for a plain user, so they're only exposed to privileged
  // roles (tazcloud/admin/superadmin), the same level the WS cloud handlers gate on.
  if (!isPrivilegedRole(ctx.role)) return base;
