...
let _dockerBin = null;
export function getDockerBin() {
    return dockerBin();
}
function dockerBin() {
    if (_dockerBin !== null)
        return _dockerBin || null;
...
async function collectMemoryInfo() {
    try {
        const [{ stdout: vmOut }, { stdout: memOut }, { stdout: swapOut }] = await Promise.all([
...
function isDockerRunning(processes) {
...
async function collectDockerInfo(processes) {
...
let cachedDockerInfo = { daemonRunning: false, containers: [] };
let cachedProcesses = [];
export function getCachedProcesses() {
    return cachedProcesses;
}
export function getCachedDockerInfo() {
...
let intervalId = null;
let dockerIntervalId = null;
export function startMonitoring(onStats, intervalMs = 2000) {
