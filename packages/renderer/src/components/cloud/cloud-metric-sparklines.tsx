...
function useChartData(history: VpsMetricSample[] | undefined) {
  return useMemo(
    () =>
...
/** Colored-dot CPU / MEM / DISK percentages from the latest history sample. */
export function CloudMetricLegend({
  history,
  hours,
  className,
}: {
  ...
  return (
    <div className={cn("flex items-center gap-3 flex-wrap min-w-0", className)}>
      {(Object.keys(chartConfig) as MetricKey[]).map((key) => (
        <span key={key} className="inline-flex items-center gap-1 text-[10px] shrink-0">
          <span
          ...
          <span className="font-mono text-subtext0 tabular-nums">
            {hasData ? `${Math.round(latest?.[key] ?? 0)}%` : "—"}
          </span>
        </span>
      ))}
      <span className="text-[10px] text-overlay0 uppercase tracking-wide shrink-0">{suffix}</span>
    </div>
  );
}

/** Sparkline chart only (legend rendered separately on Clouds cards). */
export function CloudMetricChart({
  history,
  className,
}: {
  history: VpsMetricSample[] | undefined;
  ...
  if (!hasData) {
    return (
      <div
        className={cn("w-full h-9 rounded bg-surface0/50 border border-surface0/80", className)}
  ...
  return (
    <ChartContainer config={chartConfig} className={cn("aspect-auto h-9 w-full", className)}>
          ...
          content={
            <ChartTooltipContent
              ...
              formatter={(value, name) => [
                ...
                chartConfig[name as MetricKey]?.label ?? name,
              ]}
            />
          }
        />
...
export function CloudMetricSparklines({
  ...
  return (
    <div className="flex flex-col gap-1 mt-3 min-w-0">
      <CloudMetricLegend history={history} hours={hours} />
      <CloudMetricChart history={history} />
    </div>
  );
}
