...
import { sendClaudeStreamMessage, stopClaudeStream, openClaudeStream, closeClaudeStream, pasteClaudeStreamImage, listClaudeSessions, openClaudeChatWindow, runClaudeStreamBash, dismissClaudeStreamMessage, answerClaudeStreamAsk } from "@/store/actions/claude-stream";
...
import type { ClaudeSessionSummary, PendingAsk } from "@/store/types/claude-stream";
...
/** Claude's AskUserQuestion dialog: option pills per question, immediate submit
 *  for a lone single-select question, an explicit Answer button otherwise. Claude
 *  is blocked on the control response, so this renders above the input until
 *  answered or dismissed (typing a message also dismisses it server-side). */
function AskUserQuestionDialog({ ask, onAnswer, onDismiss }: {
  ask: PendingAsk;
        ...
        return (
          <div key={q.question} className="flex flex-col gap-1.5">
              ...
              {q.options.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-full border text-[12px] cursor-pointer transition-colors ${
                      active
            ...
            <input
              ...
              placeholder="Other…"
              className="w-full bg-surface0/40 border border-surface1 rounded px-2 py-1 text-[12px] text-text placeholder:text-overlay0 outline-none focus:border-peach/60"
            />
          </div>
        );
      })}
      {(!immediate || Object.values(other).some((v) => v.trim())) && (
        <div className="flex justify-end">
          <button
            onClick={submit}
            disabled={!complete}
            className={`px-3 py-1 rounded text-[12px] border cursor-pointer transition-colors ${
              complete
                ? "border-peach bg-peach/20 text-peach hover:bg-peach/30"
